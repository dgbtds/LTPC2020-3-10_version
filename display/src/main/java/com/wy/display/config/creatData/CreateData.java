...
public class CreateData  extends Service<Integer> {
    ...
    private  void outAdcPck(DataOutputStream dos, LtpcChannel channel, int triggerNum, long external_trigger_timestamp) throws IOException {
        ...
        if (channel.getSourceBoardNum() != 0) {
            ...
            for(int i=0;i<288;i++){
                int nextInt = random.nextInt(500)+500;
    ...
    protected Task<Integer> createTask() {
        return new Task<Integer>() {
            ...
            protected Integer call() throws Exception {
                ...
                long External_trigger_timestamp=System.currentTimeMillis();
                if (trigger<1){
                    throw new RuntimeException("trigger wrong!!!");
                }
                long timeSimulation=0;
                for(int i=0;i<trigger;i++){
                    updateProgress(i,trigger);
                    outEtdPck( dos,i,External_trigger_timestamp+timeSimulation);
                    for (int j=0;j<ltpcChannels.size();j++) {
                        timeSimulation++;
                        outAdcPck(dos, ltpcChannels.get(j),i,External_trigger_timestamp+timeSimulation);
                    }
                    timeSimulation+=10;
                }
                dos.close();
