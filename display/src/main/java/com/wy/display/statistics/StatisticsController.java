...
//    private static DataSource dataSource;
    ...
    private long pckNum ;
    private String getDataView(){
        if (spark==null){
            return "sparkSession is null";
        }
...
//        } );
        pckNum = spark.sql("select count(*) as count from test ").select("count").first().getInt(0);
        String stringBuilder = spark.sql("select trigger, count(*) from test group by trigger order by trigger ").showString(100, 0, false) +
                spark.sql("select count(*) from test group by trigger").showString(10, 0, false) +
                spark.sql("select board, count(*) from test group by board order by board ").showString(100, 0, false);
    ...
    private void initialize() {
        ...
        boardHitMap.setOnAction(e -> {
                    ...
                    HashMap<Integer, LtpcSourceBoard> map = LtpcDetector.SourceBoardChannelsMap;
                    PieChart finalPieChart = pieChart;
                    map.forEach((k, v) -> {
                        PieChart.Data data = new PieChart.Data(k + "", v.clickCount);
                        finalPieChart.getData().add(data);
                    });
                    pieChart.setTitle("Board Clicked pieChart");
                    pieChart.getData().forEach(data -> {
                        Node node = data.getNode();
                        String format = String.format("board: %s , Proportion: %.2f%%", data.getName(), (double) data.getPieValue() * 100 /pckNum);
                        ...
                        node.setOnMouseClicked(event1 -> {
                            ...
                            ltpcSourceBoard.list.forEach(c -> {
                    ...
                    XYChart.Series<String, Number> series = new XYChart.Series<>();
                    map.forEach((k, v) -> {
                        series.getData().add(new XYChart.Data<>(k + "", v.clickCount));
                    });
...
//    }
    ...
    public static void setSpark(SparkSession spark) {
        StatisticsController.spark = spark;
    }
}
